...
class TableWithSC {
    ...
    public void insert( String key ) {
	    ...
	    int index = hash( key );
        Node currNode = hashArray[ index ];
        Node prevNode = null;
        boolean foundKey = false;

        if( currNode == null ){
            hashArray[ index ] = new Node( key, null );
            numberItems++;
        }else{
            while( currNode != null && !foundKey ){
                if( currNode.item.equals( key ) ){
                    foundKey = true;
                }else{
                    prevNode = currNode;
            ...
            if( foundKey ){
                System.out.printf( "%s is alread in hash table.\n", key );
            }else{
                prevNode.next = new Node( key, null );
                numberItems++;
            }
        }
